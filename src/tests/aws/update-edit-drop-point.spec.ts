...
test.describe('Verify Edit Drop Point API', () => {
  ...
  test('TC-DP-01 | Verify edit drop point with valid details', async ({ request }) => {
    ...
    expect(updateResult.updatedCustomer?.body.data.customer.addresses[0].locations[0].name).toBe(
  ...
  test('TC-DP-02 | Verify edit drop point details with missing AddressID', async ({ request }) => {
  ...
  test('TC-DP-03 | Verify edit drop point details with missing name', async ({ request }) => {
