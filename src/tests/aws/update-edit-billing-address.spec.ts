...
const baselineFilePath = path.join(
  process.cwd(),
  "src/data/update-baseline/billing-address.json",
);
...
test.describe("Verify Edit Billing Address API", () => {
  ...
  test("TC-BILL-01 | Verify edit billing address with valid details", async ({
    request,
  }) => {
    ...
    console.log(
      JSON.stringify(
        updateResult.updatedCustomer?.body.data.customer.addresses,
        null,
        2,
      ),
    );
    console.log(JSON.stringify(payload.billingAddress, null, 2));
    // Get all addresses with addressType "Billing"
    const billingAddresses = Array.isArray(
      updateResult.updatedCustomer?.body.data.customer.addresses,
    )
    ...
    expect((billingAddress?.city ?? "").toUpperCase()).toBe(
      (payload.billingAddress.city ?? "").toUpperCase(),
    );
    expect((billingAddress?.state ?? "").toUpperCase()).toBe(
      (payload.billingAddress.state ?? "").toUpperCase(),
    );
  ...
  test("TC-BILL-02 | Verify edit billing address with invalid details", async ({
    request,
  }) => {
  ...
  test("TC-BILL-03 | Verify edit billing address with empty address fields", async ({
    request,
  }) => {
  ...
  test("TC-BILL-04 | Verify edit billing address with partially empty address fields", async ({
    request,
  }) => {
  ...
  test("TC-BILL-05| Verify edit billing address with non-US address", async ({
    request,
  }) => {
