...
export async function createCustomer(request: APIRequestContext, payload: Payload) {
  ...
  const errorMessage = body?.error || body?.message === 'Error' ? body?.error : null;
...
export async function pollGetRequest(request: APIRequestContext, requestID: string, maxRetries = 15, intervalSec = 5) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function browseCustomers(request: APIRequestContext, payload: any) {
  //   console.log(
